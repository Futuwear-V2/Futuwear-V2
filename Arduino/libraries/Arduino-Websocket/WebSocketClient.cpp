...
WebSocketClient::WebSocketClient(String WsPath, String WsHost){
    path = WsPath;
    host = WsHost;
}
...
void WebSocketClient::sendEncodedData(char *str, uint8_t opcode) {
    ...
    socket_client->write(mask, 4);
     
    for (int i=0; i<size; ++i) {
        msg[i] = str[i] ^ mask[i % 4];
    }
    socket_client->write((const uint8_t*)msg, size);
}

void WebSocketClient::sendBytes(const char *str, uint8_t size) {
    uint8_t mask[4];
