/*
  ...
  over WiFi using WebSocket.

  The UDP version should be used instead.
...
WiFiClient client;
WebSocketClient webSocketClient((const char*)"/ws", (const char*)"protopaja2.research.comnet.aalto.fi");

//char IMUbytes[4 * N_IMUS + 1];
uint8_t IMUbytes[20];
String encodedData;

//float previousValues[N_IMUS][3]; 
...
void loop()
{
  ...
  if (outputTimer <= millis()) {
    ...
    // Try to reconnect, if connection to the websocket server was lost.
    if (!client.connected()) {
#ifdef SERIAL_OUTPUT
      Serial.println("Lost connection to the server. Attempting to reconnect.");
#endif
      connnectServer();
    }

#ifdef SERIAL_OUTPUT
    // Print data received from the server, if available
    if (client.available()) {
    ...
    if (change) {
      encodedData = b64.encode(IMUbytes, 200);
      webSocketClient.sendData(encodedData);
#ifdef SERIAL_OUTPUT
      Serial.print(encodedData); Serial.println();
    } else {
...
void connnectServer()
{
  if (client.connect("protopaja2.research.comnet.aalto.fi", 80)) {
#ifdef SERIAL_OUTPUT
    Serial.println("Server connected");
#endif
    setRGB(0, 1, 0);
    uint32_t value = 10;
    // Connected to server, attempt websocket handshake
    connectWebSocket();
  } else {
    // Connection failed, wait 3 seconds and retry
#ifdef SERIAL_OUTPUT
...
void connectWebSocket()
{
  if (webSocketClient.handshake(client)) {
...
#endif
    // Let the server know that the client connected as shirt
    delay(1000);
    webSocketClient.sendData("shirt");
    // client.flush();
#ifdef SERIAL_OUTPUT
    ...
    Serial.print("Server response: "); Serial.println(data);
#else
    webSocketClient.getData(NULL);
#endif
    setRGB(0, 1, 0);
    // client.setNoDelay(true);
  } else {
    // Handshake failed, wait 3 seconds and try to reconnect
#ifdef SERIAL_OUTPUT
